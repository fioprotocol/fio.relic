...
void Writer::sendTracesBatch()
{
	...
	//	transactions.push_back(std::string("(" + std::to_string(t.seq)).append("," + std::to_string(t.block_num)).append(",'" + t.block_time + "'").append(",'" + t.trx_id + "'").append(",'" + t.trace + "')"));
	//insert_transactions += boost::algorithm::join(transactions, ",");
	...
	for (auto t : insertTransactions)
	{
		...
		sth_insert_transactions->setString(5, t.trace);
